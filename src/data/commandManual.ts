...
export const COMMAND_MANUAL_ENTRIES: ManualEntry[] = [
  ...
  {
    id: 'et-agent-terminal',
    type: 'command',
    category: 'easyterminal',
    title: '在终端里调用 EasyTerminal Agent',
    summary: '让内建 Agent 在当前终端会话里调用 prompt、skill、workflow、context 等能力。',
    purpose: '适合直接在 EasyTerminal 底部输入框里发自然语言任务，而不是打开 DevTools 手动调 IPC。',
    syntax: '/agent <任务描述>',
    example: '/agent 帮我搜索当前项目里和 context runtime 相关的能力，并总结下一步建议',
    actionCommand: '/agent 帮我搜索当前项目里和 context runtime 相关的能力，并总结下一步建议',
    pinLabel: '/agent',
    note: '该命令会走 EasyTerminal 内建 Agent Runtime，不会把命令发给真实 shell。',
    tags: ['easyterminal agent', '/agent', 'context', 'workflow', 'skill', 'prompt'],
    verification: 'general',
  },
  {
    id: 'et-agent-skill',
    type: 'command',
    category: 'easyterminal',
    title: '在终端里搜索或查看 Skill',
    summary: '直接在终端中搜索 Skill，或根据 id 查看单个 Skill 的详情。',
    purpose: '适合把 Skill 检索能力拉到主工作流里，减少在面板和终端之间切换。',
    syntax: '/skill <query>  或  /skill get <skillId>',
    example: '/skill context optimization',
    actionCommand: '/skill context optimization',
    pinLabel: '/skill',
    note: '默认会执行语义搜索；`get` 适合在你已经拿到某个 Skill id 之后继续查看。',
    tags: ['/skill', 'skill search', 'skill get', 'agent tools'],
    verification: 'general',
  },
  {
    id: 'et-agent-prompt',
    type: 'command',
    category: 'easyterminal',
    title: '在终端里搜索、优化或渲染 Prompt',
    summary: '支持搜索 Prompt、优化草稿，或按变量渲染已有 Prompt 模板。',
    purpose: '适合把 Prompt 工程直接放进终端工作流，而不是切到独立面板再复制粘贴。',
    syntax: '/prompt <query>  /prompt optimize <draft>  /prompt render <promptId> {"var":"value"}',
    example: '/prompt optimize 帮我写一个上下文压缩提示词草稿',
    actionCommand: '/prompt optimize 帮我写一个上下文压缩提示词草稿',
    pinLabel: '/prompt',
    note: '渲染命令里的变量使用 JSON 格式，便于后续与工作流或 Agent 共享。',
    tags: ['/prompt', 'prompt search', 'prompt optimize', 'prompt render'],
    verification: 'general',
  },
  {
    id: 'et-agent-workflow',
    type: 'command',
    category: 'easyterminal',
    title: '在终端里搜索或执行 Workflow',
    summary: '支持在终端里搜索 Workflow、查看定义，或传 JSON 变量直接执行。',
    purpose: '适合把 EasyTerminal 的工作流引擎接到终端 Agent 流程里，做真正可执行的自动化。',
    syntax: '/workflow <query>  /workflow get <workflowId>  /workflow run <workflowId> {"input":"value"}',
    example: '/workflow run summarize_context {"topic":"EasyTerminal context runtime"}',
    actionCommand: '/workflow run summarize_context {"topic":"EasyTerminal context runtime"}',
    pinLabel: '/workflow',
    note: '执行时会使用当前已配置的推理模型和 embedding 模型。',
    tags: ['/workflow', 'workflow search', 'workflow run', 'workflow execute'],
    verification: 'general',
  },
  {
    id: 'et-agent-context',
    type: 'command',
    category: 'easyterminal',
    title: '在终端里查看或写入 Context',
    summary: '在终端里直接查看上下文资产、结构化记录、快照和当前会话事件，也可以把笔记存进 Context Vault。',
    purpose: '适合把 Context Runtime 拉进日常开发流程，减少“看一处、写一处”的上下文割裂。',
    syntax: '/context list  /context records [query]  /context snapshots  /context events  /context capture <text>',
    example: '/context capture 当前目标是让终端 Agent 能直接调用 EasyTerminal 的 workflow 与 prompt 能力',
    actionCommand: '/context capture 当前目标是让终端 Agent 能直接调用 EasyTerminal 的 workflow 与 prompt 能力',
    pinLabel: '/context',
    note: '`events` 默认查看当前终端标签页对应的 session 事件。',
    tags: ['/context', 'context list', 'context capture', 'context snapshots', 'context records'],
    verification: 'general',
  },
  {
    id: 'et-agent-help',
    ...
    category: 'easyterminal',
    title: '打开终端 Agent 教程',
    summary: '直接从终端里呼出带 Agent 命令说明的手册。',
    purpose: '适合在敲命令时忘了格式，立即查看而不离开当前工作流。',
    syntax: '/help agent  或  /et help',
    example: '/help agent',
    actionCommand: '/help agent',
    pinLabel: '/help agent',
    tags: ['/help agent', '/et help', 'manual', 'tutorial'],
    verification: 'general',
  },
  {
    id: 'linux-pwd',
